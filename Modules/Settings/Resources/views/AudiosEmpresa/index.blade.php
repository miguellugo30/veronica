                    ...
                    <tbody>
                        @foreach( $audios as $audio )
                            <tr data-id="{{ $audio->id }}" style="cursor:pointer">
                                ...
                                <td><audio controls preload="metadata"><source src='http://localhost/Nimbus/storage/app/public/audios/{{ $audio->Empresas_id }}/<?php strtok($audio->ruta,'/'); echo strtok('/'); ?>' type="audio/wav">Tu navegador no soporta audio, favor de obtener la ultima version</audio></td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
